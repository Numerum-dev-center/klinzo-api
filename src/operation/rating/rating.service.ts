...
export class RatingService {
  ...
  async create(dto: CreateRatingDto, requestingUserTrackingId: string): Promise<RatingResponse> {
    ...
    // b. Authorization
    if (collectionEvent.subscription.user.trackingId !== requestingUserTrackingId) {
      throw new ForbiddenException('You can only rate your own collections');
    }

    // c. Validation status
    if (collectionEvent.status !== CollectionStatus.VALIDATED) {
      throw new BadRequestException('Rating is only allowed after the collection has been validated');
    }
  ...
  async findAll(pageOptionsDto: PageOptionsDto): Promise<PageDto<RatingResponse>> {
    ...
    const entities = ratings.map(r => new RatingResponse({
  ...
  async findAllByCollector(collectorTrackingId: string, pageOptionsDto: PageOptionsDto): Promise<PageDto<RatingResponse>> {
  ...
  async findOne(trackingId: string): Promise<RatingResponse> {
