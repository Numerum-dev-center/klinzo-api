...
export class FinancialDocumentsService {
  ...
  async findAll(
    ...
    const entities = documents.map(
      (d) =>
        new FinancialDocumentResponse({
  ...
  async findAllByCollector(
    ...
    if (
      requestingUser.role === Role.ADMIN_COLLECTEUR ||
      requestingUser.role === Role.AGENT_COLLECTEUR
    ) {
      ...
      if (!user || user.collectorId !== collector.id) {
        throw new ForbiddenException(
          'You can only access financial documents for your own collector entity',
        );
      }
    }
  ...
  async markAsPaid(trackingId: string): Promise<FinancialDocumentResponse> {
    ...
    if (document.status !== FinancialDocumentStatus.PENDING) {
      throw new BadRequestException(
        'Only PENDING documents can be marked as PAID',
      );
    }
